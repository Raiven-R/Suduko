...
void DrawSudokuGrid(int grid[N][N], int selectedRow, int selectedCol) 
{
    ...
    // Draw the grid lines
     for (int i = 0; i <= N; i++) 
     {
         ...
         DrawLineEx((Vector2) {offsetX + i * CELL_SIZE, offsetY}, (Vector2) {offsetX + i * CELL_SIZE, offsetY + GRID_HEIGHT}, thickness, BLACK);
         DrawLineEx((Vector2){offsetX, offsetY + i * CELL_SIZE}, (Vector2) {offsetY + GRID_HEIGHT, offsetY + i * CELL_SIZE}, thickness, BLACK);
     }
